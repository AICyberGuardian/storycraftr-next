...
- **Fail-closed narrative state loading** — Corrupt or invalid `outline/narrative_state.json` now raises a state validation error instead of silently loading an empty/partial snapshot.
- **Direct-write bypass now dual-gated** — `storycraftr chapters chapter` now requires both `--unsafe-direct-write` and `STORYCRAFTR_ALLOW_UNSAFE=1` for developer-only usage.
- **Severe canon violations force immediate coherence gate** — `BookEngine` now executes the coherence gate immediately when severe violation checks fail, regardless of interval cadence.
- **Narrative snapshot schema parity fields** — `NarrativeStateSnapshot` now includes `relationships` and `world_facts` as first-class fields.
- **Explicit validator dependency contract** — `jsonschema` is now an explicit runtime dependency in `pyproject.toml` to guarantee schema-validation availability for validator report enforcement.
- **Retry-time model escalation ladder** — `storycraftr book` now rotates OpenRouter ranked role models (`batch_prose`, `batch_editing`, and `coherence_check` for semantic failures) after repeated quality-validation retries, reducing same-model retry stasis during autonomous runs.
- **Scene density repair directive injection** — Scene retries triggered by minimum-word failures now include an explicit craft correction instruction to deepen Goal/Conflict/Disaster/Sequel content instead of adding filler text.
- **Stress-test rankings fixture** — Added `examples/rankings_stress_weak_primary.json` for reproducible model-escalation soak tests with a deliberately weaker prose primary.

- **Phase 7J ruggedization for long-form generation** — `BookEngine` now applies explicit prose completeness checks (`min_words` + truncation suffix guard) at scene and chapter stages, with fail-closed stitch fallback to concatenated edited scenes.
- **Dedicated chapter completeness module** — Added `storycraftr/agent/chapter_validator.py` for duplicate-loop detection and bounded retry validation, integrated directly before `BookEngine` state extraction/commit to prevent malformed prose from poisoning canon/state artifacts.
- **Planner-quality fail-closed gate** — `BookEngine` now validates scene planner directives (`goal/conflict/stakes/outcome`) for minimum signal before drafting, halting weak/degenerate free-tier planner outputs early.
- **Per-chapter guard diagnostics line** — `storycraftr book` now prints a compact runtime diagnostics summary for each chapter (`directive_quality`, `chapter_validation_attempts`, `last_retry_reason`, `state_signal`) to speed up soak-run triage.
- **Stage packet persistence + acceptance contract gate** — `storycraftr book` now writes deterministic per-chapter packet artifacts under `outline/chapter_packets/chapter-<nnn>/` (`outline_context.md`, `scene_plan.json`, scene draft/edit markdown, `stitched_chapter.md`, `state_patch.json`, `canon_delta.yml`, `diagnostics.json`, `validator_report.json`) and fails closed before commit if acceptance checks fail.
- **Scene-level validator contracts and reports** — `storycraftr book` now computes per-scene acceptance checks (directive fields, draft/edit non-empty, truncation guard, and provider-gated minimum scene words), fails closed on scene contract violations before commit, and persists `scene_<n>_validator_report.json` artifacts in each chapter packet.
- **Stage-by-stage validator report schema** — `storycraftr book` now records explicit precommit stage contracts in packet `validator_report.json` (`outline`, `scene_plan`, `scene_draft`, `scene_edit`, `stitch`, `state_extract`, `semantic_review`) and preserves precommit details in postcommit reports for durable validator handoff/audit context.
- **Machine-validated validator report contract** — Added `storycraftr/config/validator_report.schema.json` and fail-closed runtime schema checks before writing precommit/postcommit `validator_report.json`, ensuring packet report shape remains contract-safe for downstream tooling.
- **Fail-closed coherence gate + autonomous semantic requirement** — Coherence auditing now uses strict JSON `PASS`/`FAIL` verdicts that can block chapter progression, and CLI `storycraftr book --yes` now requires `enable_semantic_review=true` for real-provider runs (`openai`/`openrouter`/`ollama`).
- **Rollback safety for commit artifacts** — `storycraftr book` commit flow now performs best-effort rollback of `outline/narrative_state.json`, `outline/canon.yml`, and `chapters/chapter-<n>.md` if any commit-step write fails, reducing cross-file partial-persistence drift.
- **Retry ladder hardening + bypass guard** — Chapter validation retries now regenerate scene draft/edit artifacts between attempts, and `storycraftr chapters chapter` requires both explicit `--unsafe-direct-write` and `STORYCRAFTR_ALLOW_UNSAFE=1` to prevent accidental bypass of validator-gated runtime paths.
- **Strict autonomous enforcement mode** — `storycraftr book --yes` on real providers now runs with mandatory semantic validation, per-chapter coherence gates, required severe-canon checks, and explicit fail-closed preflight checks before runtime orchestration starts.
- **Strict planner directive schema validation** — Scene planner output is now validated against exact JSON shape (`goal`, `conflict`, `stakes`, `outcome` only) and minimum-content checks before `SceneDirective` construction, with bounded repair attempts and fail-closed halt on schema miss.
- **Run-level audit artifact export** — Successful `storycraftr book` runs now emit consolidated summaries in `outline/book_audit.json` and `outline/book_audit.md`, including chapter packet paths, acceptance/stage-contract outcomes, semantic/coherence diagnostics, and retry reasons.
- **Phase 7L semantic reviewer loop** — Added an optional LLM-powered semantic reviewer pass (`storycraftr/prompts/reviewer_rules.md`) wired before state extraction. When `enable_semantic_review=true`, chapters are retried fail-closed on reviewer `FAIL` responses, and diagnostics include semantic review status/reason.
- **OpenRouter truncation hardening** — OpenRouter `ChatOpenAI` clients now receive explicit `max_tokens=4000` to prevent provider default token ceilings from clipping chapter prose.
...
- **`/context prompt` stage diagnostics** — Added a dedicated TUI diagnostics command to inspect per-stage prompt composition (`planner`, `drafter`, `editor`) with section inclusion/pruning markers and estimated token usage.
- **Planner directive debug toggle** — Added `/context prompt debug [on|off]` so operators can explicitly log resolved planner `SceneDirective` fields (`goal`, `conflict`, `stakes`, `outcome`) after each planner stage.
- **Fail-closed planner fallback reuse** — When planner JSON remains invalid after the bounded repair pass, generation now reuses the last validated `SceneDirective` if available and emits a warning instead of silently continuing with malformed planner output.
- **Phase 6B: `/context memory explain` diagnostics view** — Added a detailed TUI diagnostics mode that renders the latest memory recall snapshot with source order and item-level source mapping (`[source] text`) so operators can inspect exactly which retrieval stage contributed each selected memory line.
- **Phase 6B: memory retrieval telemetry diagnostics** — Runtime memory diagnostics now expose latest retrieval telemetry (`hits_returned`, `queries_run`, `queries_attempted`, and `hits_by_source`) to support operator tuning. The telemetry is surfaced in both TUI `/context memory` and CLI `storycraftr memory status` text output while JSON output preserves full structured payloads.
- **Phase 6B: storyline-aware weighted memory retrieval** — Memory recall now prioritizes continuity-sensitive queries after the user's prompt: active chapter and previous chapter carry-over, active scene cues, active arc cues, then broader character-state and plot-thread signals before generic intent/event fallback. Retrieval remains fail-closed and deduplicated while using chapter-scoped filters where available.
- **Phase 6B: model-aware memory token budgeting** — Memory recall budgets now scale dynamically based on the active model's context window, integrated with OpenRouter live discovery. Larger models (e.g., 128k context) receive proportionally larger memory budgets (up to 1280 tokens) to utilize available capacity, while smaller models (e.g., 8k context) use conservative budgets (160+ tokens) to preserve space for critical prompt sections. Budget is computed as ~2% of context window with floor/ceiling bounds, ensuring memory context adapts to model capabilities without manual tuning.
- **Phase 6B: memory persistence diagnostics** — Memory write failures are now surfaced immediately in the TUI output pane when memory is enabled but persistence fails. Last persist status (`success` or `failed`) is tracked and displayed in `/context memory` diagnostics for operator visibility. This replaces silent error capture with actionable feedback when long-term memory storage encounters issues.
- **Phase 6B: query-aware memory retrieval** — Enhanced memory recall to use the user's current prompt for semantic relevance ranking. `NarrativeMemoryManager.get_context_items(...)` now accepts an optional `query` parameter and uses it as the primary retrieval query before falling back to generic intent/event queries. This improves memory recall precision by surfacing context directly related to what the user is actually asking for.
- **Phase 6B: memory-context budget guard in prompt assembly** — Added deterministic token capping for memory-context lines in `NarrativeStateEngine.get_memory_context(...)` so recalled Mem0 snippets cannot crowd out higher-priority prompt sections before existing budget-pruning logic runs.
- **Phase 6 foundation: optional Mem0 memory layer + expanded deterministic scene directives** — Added `storycraftr/agent/memory_manager.py` with a fail-closed optional Mem0 adapter (Chroma-backed local storage under internal state paths), runtime turn persistence, and prompt-context retrieval hooks. Integrated memory enrichment into `NarrativeStateEngine` prompt composition and best-effort post-generation persistence in the TUI loop. Expanded deterministic scene planning to include explicit `stakes` and `ending_beat` fields so generation receives stronger scene-level objectives before prose output.
- **Phase 6 hardening: memory diagnostics and operator controls** — Added Mem0 runtime toggles (`STORYCRAFTR_MEM0_ENABLED`, `STORYCRAFTR_MEM0_FORCE_PROVIDER`, `STORYCRAFTR_MEM0_FORCE_OPENROUTER`), TUI diagnostics subcommands `/context memory` and `/context refresh-memory`, and new CLI memory surface (`storycraftr memory status|search|remember`) including automation-friendly `storycraftr memory status --format json` and `storycraftr memory search --format ndjson`.
...
- **DSVL Phase 4: Extraction verification and bounded retry repair** — Added dependency-aware verification in `state_extract_impl` to fail closed on invalid extracted operations, perform one deterministic operation-order retry, and drop unsafe operations (for example dead-character movement). CLI/TUI extraction surfaces now report verification status, dropped operations, and verification issues.
- **DSVL Phase 5: Mode-gated state critic regeneration** — Extended TUI mode-aware generation with one bounded critic retry in `storycraftr/tui/app.py`: when extraction verification reports unsafe state transitions, hybrid/autopilot flows request a single constrained revision before post-generation hooks, preserving fail-closed behavior and bounded autonomy.
...
- **Strict rankings validation command** — Added `storycraftr models validate-rankings` (`--refresh`, `--format text|json`) to fail closed on invalid `storycraftr/config/rankings.json` and surface actionable validation errors for model-routing configuration.
- **Strict rankings schema/runtime contract** — Added and enforced strict rankings configuration semantics for OpenRouter routing (`storycraftr/config/rankings.schema.json` + runtime validation in `storycraftr/llm/factory.py`): exact role keys, canonical `fallbacks`, constrained model-id format, `repair_json` primary allowlist, `primary` exclusion from fallbacks, and live discovery/context-limit checks.
...
- **DSVL Phase 1C: Rule-Governed Patch Validation & Application** — Added `StateValidationError` exception, `PatchOperation`, and `StatePatch` dataclasses for validated state transitions. `validate_patch()` enforces business rules: dead characters cannot change location, location references must exist, cannot remove locations with characters, cannot add duplicate entities. `apply_patch()` atomically applies multi-operation patches with version increments and timestamp updates. Comprehensive test suite with 14 patch validation tests ensures rule enforcement.
- **DSVL Phase 2A: Persistent Audit Trail Logging** — Added `state_audit.py` module with `AuditEntry` and `StateAuditLog` classes for append-only JSONL audit logging. Every `apply_patch()` operation automatically logs full patch + resulting changeset with timestamp, actor, and metadata. `query_entries()` supports filtering by entity ID, entity type, operation type, time range, and result limit. Audit logging can be disabled via `enable_audit=False` flag. Comprehensive test suite with 16 audit trail tests including integration tests ensures reliable change tracking and queryability.
- **DSVL Phase 2B: TUI Audit Trail Integration** — Added `/state audit` TUI command for interactive audit history inspection with filtering support. Users can query audit entries by entity ID (`entity=<id>`), entity type (`type=<character|location|plot_thread>`), and result limit (`limit=<n>`). Command displays timestamp, operation type, actor, patch operation count, changeset modification count, and state version for each entry. Implemented subcommand routing pattern in `_handle_state_command()` preserving backward compatibility with `/state` (default state view). Comprehensive test suite with 11 tests covering all filter combinations, error handling for invalid inputs, and graceful degradation when audit logging is disabled.
- **DSVL Phase 2C: Version-Aware Prompt Injection** — Modified `render_prompt_block()` in `narrative_state.py` to inject version metadata header before JSON payload. Header format: `[Narrative State v{version} as of {timestamp}]`. Version and timestamp are extracted from `NarrativeStateSnapshot` model fields. Header is not counted toward `max_chars` truncation limit (applies to JSON only). Empty state continues to return empty string with no header. LLMs can now reference specific narrative state versions in their responses for better continuity tracking. Comprehensive test suite with 5 tests validates header format, empty state handling, truncation behavior, and version increment tracking.
