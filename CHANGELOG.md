...
- Added a small in-repo model-context registry (`storycraftr/llm/model_context.py`) with conservative defaults and explicit handling for `openrouter/free` and `:free` variants to stabilize budgeting under volatile free-model routing.
...
- Added TUI diagnostics for compaction transparency: `/summary` (`/summary clear`) and `/context` now expose rolling summary and prompt-context composition details for long sessions.
